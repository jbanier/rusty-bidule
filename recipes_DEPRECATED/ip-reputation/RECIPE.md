...
Instructions:
The goal is to assess one or more IP addresses and determine whether each one is internal to the organization or a related subsidiary, or external. Then gather enough context to explain what the asset is, what services are exposed, and whether the hosting or exposure looks suspicious.
...
- Classify the address as `Internal` if it belongs to the organization or a clearly related subsidiary or brand.
...
- For internal or related assets, use the RunZero MCP against the configured internal organization.
...
- For external assets, use web search to evaluate the hosting provider or owner named by `whois`.
