#!/usr/bin/env python3
"""Manage a Google Calendar read-only OAuth token."""
...
def cmd_login(credentials_file: str | None, open_browser: bool) -> None:
    ...
    token = authorize_user(client, [CALENDAR_SCOPE], open_browser=open_browser)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Manage the Google Calendar read-only token used by the google-calendar-read skill.\n\n"
